...
class TestBuildSampler:
    def test_default_params_returns_callable(self):
        ...
        params = mlx_engine_pb2.SamplingParams()
    ...
    def test_temperature_and_top_p(self):
        ...
        params = mlx_engine_pb2.SamplingParams(temperature=0.8, top_p=0.95)
        sampler = MlxEngineServicer._build_sampler(params)
        logits = mx.ones((100,))
        token = sampler(logits)
    ...
    def test_top_k(self):
        ...
        params = mlx_engine_pb2.SamplingParams(temperature=1.0, top_k=5)
        sampler = MlxEngineServicer._build_sampler(params)
        logits = mx.ones((100,))
        token = sampler(logits)
        assert 0 <= token.item() < 100
...
class TestBuildLogitsProcessors:
    ...
    def test_repetition_penalty(self):
        ...
        params = mlx_engine_pb2.SamplingParams(repetition_penalty=1.2)
        processors = MlxEngineServicer._build_logits_processors(params)
    ...
    def test_logit_bias(self):
        ...
        params = mlx_engine_pb2.SamplingParams(logit_bias={5: 10.0, 10: -10.0})
        processors = MlxEngineServicer._build_logits_processors(params)
        assert len(processors) >= 1
...
class TestBuildStateMachine:
    ...
    def test_stop_token_ids(self):
        ...
        params = mlx_engine_pb2.SamplingParams(stop_token_ids=[128001, 128009])
        eos_token_ids = []
        sm = MlxEngineServicer._build_state_machine(params, eos_token_ids)
        assert sm is not None
    ...
    def test_ignore_eos_excludes_eos(self):
        ...
        params = mlx_engine_pb2.SamplingParams(ignore_eos=True, stop_token_ids=[999])
        eos_token_ids = [2]
        sm = MlxEngineServicer._build_state_machine(params, eos_token_ids)
        assert sm is not None
