...
impl Glm4MoeParser {
    ...
    fn parse_argument_value(value_str: &str, arg_type: Option<&str>) -> (Value, bool) {
        let value_str = value_str.trim();

        if let Ok(json_val) = serde_json::from_str::<Value>(value_str) {
            let normalized = match arg_type {
                Some("string") => match json_val {
                    ...
                    other => Value::String(match other {
                        ...
                        Value::Null => value_str.to_string(),
    ...
    fn parse_arguments(
        ...
        for capture in self.arg_extractor.captures_iter(args_text) {
            ...
            let (value, _is_good_json) = Self::parse_argument_value(value_str, arg_type);
...
impl ToolParser for Glm4MoeParser {
    ...
    async fn parse_incremental(
        ...
        if start.is_none() {
            ...
            self.buffer.clear();
            // If we're in the middle of streaming, don't return buffered text fragments.
            let normal_text = if self.current_tool_id >= 0 {
                String::new()
            } else {
                current_text.clone()
        ...
        if end.is_some() {
            ...
            loop {
                ...
                if !parsed_tools.is_empty() {
                    // Each block should correspond to one complete tool call.
            ...
            // Keep any incomplete trailing content for the next incremental parse.
